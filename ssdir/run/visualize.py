...
def visualize_latents(
    images: torch.Tensor, model: SSDIR, n_objects: int = 8
) -> plt.Figure:
    ...
    with torch.no_grad():
        ...
        for idx, (image, reconstruction, what, where, present, depth) in enumerate(
            zip(images, reconstructions, z_what, z_where, z_present, z_depth)
        ):
